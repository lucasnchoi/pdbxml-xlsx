...
def select_file():
    """Open file dialog to select an XML/PDBXML file."""
    file_path = filedialog.askopenfilename(filetypes=[("PDBXML Files", "*.pdbxml"), ("XML Files", "*.xml")])
    if file_path:
        file_entry.delete(0, 'end')
        file_entry.insert(0, file_path)
        default_output = os.path.splitext(file_path)[0] + "_report.xlsx"
        output_entry.delete(0, 'end')
        output_entry.insert(0, default_output)

def select_output():
    """Open save dialog to choose output file path."""
...
def convert_file():
    """Handles the XML to Excel conversion process."""
    input_file = file_entry.get()
    output_file = output_entry.get()
    graph_bool = graph_var.get()

    if not input_file or not os.path.exists(input_file):
        messagebox.showerror("Error", "Please select a valid XML/PDBXML file.")
        return
...
root = Tk()
root.title("PDBXML to Excel Converter")
root.geometry("500x250")

Label(root, text="Select XML/PDBXML File:").pack(pady=5)
...
Button(root, text="Convert", command=convert_file, fg="white", bg="green").pack(pady=20)
